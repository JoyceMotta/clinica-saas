...
import { buscarClienteLocalPorId, atualizarClienteLocal } from '@/lib/local-storage-clientes';
...
export default function EditarClientePage() {
  ...
  const [v, setV] = useState<ClienteInput | null>(null);
  ...
  useEffect(() => {
    ...
    setNomeOriginal(cliente.nome);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { id: _id, createdAt: _c, updatedAt: _u, ...campos } = cliente;
    setV(campos as ClienteInput);
  }, [id]);

  // ── Helpers de estado ─────────────────────────────────────────────────────
  const set = (f: keyof ClienteInput, val: string | boolean) =>
    setV((prev) => prev ? { ...prev, [f]: val } : prev);
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setErrors({});
    setIsPending(true);
    atualizarClienteLocal(id, v);
    setSavedOk(true);
    setTimeout(() => router.push(`/clientes/${id}`), 1200);
  }
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <nav className="flex items-center gap-2 text-sm text-gray-400">
          <Link href="/clientes" className="hover:text-navy transition-colors">Clientes</Link>
          <span>/</span>
          <span className="text-navy font-medium">Editar</span>
        </nav>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center space-y-4">
          ...
          <p className="text-lg font-bold text-navy">Cliente não encontrado</p>
          <p className="text-sm text-gray-500">Nenhum cliente com o ID informado foi localizado.</p>
          <Link
            href="/clientes"
    ...
    return (
      <div className="max-w-3xl mx-auto flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-lg font-bold" style={{ color: '#1B2A4A' }}>Dados salvos com sucesso!</p>
          <p className="text-sm text-gray-500">Redirecionando para a ficha do cliente…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400">
        <Link href="/clientes" className="hover:text-navy transition-colors">Clientes</Link>
        <span>/</span>
        <Link href={`/clientes/${id}`} className="hover:text-navy transition-colors truncate max-w-[180px]">
          {nomeOriginal}
        </Link>
        <span>/</span>
        <span className="text-navy font-medium">Editar</span>
      ...
      {/* Cabeçalho */}
      <div>
        <h1 className="text-3xl font-bold" style={{ color: '#1B2A4A' }}>Editar Cliente</h1>
        <p className="text-gray-500 mt-1 text-sm">
          Altere os dados de <strong>{nomeOriginal}</strong>. Cada campo editado será registrado no histórico de auditoria.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-6 md:px-8 pt-8 pb-0">
        <form onSubmit={handleSubmit} noValidate>
          <div className="space-y-10 pb-6">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor" style={{ color: '#C9A84C' }}>
              ...
              <div>
                ...
                <p className="text-xs mt-0.5 text-gray-600 leading-relaxed">
                  As alterações serão registradas no <strong>log de auditoria</strong> com data, hora e responsável.
                </p>
              </div>
            </div>
            ...
            {submitted && errCount > 0 && (
              <div className="rounded-xl bg-red-50 border border-red-200 p-4 space-y-3">
                <p className="text-sm font-bold text-red-700 flex items-center gap-2">
                  ...
                  {errCount === 1 ? '1 campo obrigatório não preenchido:' : `${errCount} campos obrigatórios não preenchidos:`}
                </p>
                  ...
                  {Object.entries(errors)
                    ...
                    .map(([k, msgs]) => (
                      <li key={k} className="flex items-start gap-2 text-xs text-red-600">
                        ...
                        <span>
                          <strong>{FIELD_LABELS[k as keyof typeof FIELD_LABELS] ?? k}</strong>
                          {' — '}
              ...
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
                ...
                {!v.menorDeIdade && (
                  <p className="mt-2 text-xs text-amber-700 ml-14 leading-relaxed">
                    Ao ativar, todos os dados do Responsável Legal serão exigidos por obrigação legal e clínica.
                  </p>
                )}
              </div>
            </section>
            ...
            {v.menorDeIdade && (
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <F label="Nome completo do responsável" req err={errors.nomeResponsavel?.[0]} cls="sm:col-span-2">
                    <input
                  ...
                  <F label="Grau de parentesco com o menor" req err={errors.parentescoResponsavel?.[0]}>
                    <Sel
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 ml-auto">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="flex items-center justify-center gap-2 px-7 py-2.5 text-sm font-bold rounded-lg transition-all hover:opacity-90 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed min-w-[170px]"
